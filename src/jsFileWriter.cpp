...
void jsFileWriter::Close() {
  ...
  m_jsReader = NULL; // it points to outside reader, so just NULL it
...
int jsFileWriter::initDataType(const std::string dataType, std::string dataFormat, bool isMapped, int nextends, std::string vpath) {
  ...
  if(!vpath.empty()) {
    ...
    if(vFolderPathRest[0] == '/' && vpath[vpath.length() - 1] == '/') vFolderPathRest = vFolderPathRest.substr(1, vFolderPathRest.length());
...
int jsFileWriter::initGridAxis(int axisInd, std::string axislabel, std::string units, std::string domain, long length, long logicalOrigin,
    long logicalDelta, double physicalOrigin, double physicalDelta) {
  if(axisInd >= 0 && axisInd < m_numDim) {
    m_gridDef->getAxisPtr(axisInd)->Init(AxisLabel(axislabel, axislabel), Units(units), DataDomain(domain), length, logicalOrigin,
                                         logicalDelta, physicalOrigin, physicalDelta, "", "");
    return JS_OK;
  } else {
...
void jsFileWriter::addSurveyGeom(int minILine, int maxILine, int minXLine, int maxXLine, double xILine1End, double yILine1End,
    double xILine1Start, double yILine1Start, double xXLine1End, double yXLine1End, int resetOrigin) {
  int nIL = maxILine - minILine + 1;
  int nXL = maxXLine - minXLine + 1;
  m_customProps->survGeom.setGeom(minILine, maxILine, nIL, minXLine, maxXLine, nXL, xILine1End, yILine1End, xILine1Start, yILine1Start,
                                  xXLine1End, yXLine1End, resetOrigin);
}

void jsFileWriter::addSurveyGeom(int minILine, int maxILine, int nIL, int minXLine, int maxXLine, int nXL, double xILine1End,
    double yILine1End, double xILine1Start, double yILine1Start, double xXLine1End, double yXLine1End, int resetOrigin) {
  m_customProps->survGeom.setGeom(minILine, maxILine, nIL, minXLine, maxXLine, nXL, xILine1End, yILine1End, xILine1Start, yILine1Start,
                                  xXLine1End, yXLine1End, resetOrigin);
}
...
int jsFileWriter::updateGridAxis(int axisInd, long length, long logicalOrigin, long logicalDelta, double physicalOrigin,
...
int jsFileWriter::Init(jsFileReader *jsReader) {
  ...
  *m_customProps = *(jsReader->m_customProps);
  ...
  // m_virtualFolders.clear(); // set to use primary
...
int jsFileWriter::Initialize(const std::string _filename) {
...
int jsFileWriter::Initialize() {
  ...
  m_frameSize = m_numTraces * (long)m_compess_traceSize;
  m_traceFileSize = m_TotalNumOfTr * (long)m_compess_traceSize;

  m_headerLengthBytes = m_traceProps->getRecordLength();
  m_frameHeaderSize = ((long)m_headerLengthBytes * (long)m_numTraces);
  m_headerFileSize = m_TotalNumOfTr * (long)m_headerLengthBytes;
  m_headerLengthWords = ceil(m_headerLengthBytes / 4.f);
  //   fprintf(plogfile,"m_headerLengthBytes=%d, m_numTraces=%d, m_headerFileSize=%ld\n",m_headerLengthBytes, m_numTraces, m_headerFileSize);
  ...
  TRACE_PRINTF(jsFileWriterLog, "traceLen=%d, TotalNumOfFrames=%ld, TotalNumOfTraces=%ld", m_compess_traceSize, m_TotalNumOfFrames,
               m_TotalNumOfTr);
...
int jsFileWriter::writeMetaData(const int remove) {
  ...
  for(int i = 0; i < m_TrFileExtents->getNumExtents(); i++) {
    ...
    if(m_jsReader != NULL && remove == 2) {
      ...
      // int fd = ::open(fname.c_str(), O_CREAT | O_WRONLY | O_SYNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | O_DIRECT);
      int fd = ::open(fname.c_str(), O_CREAT | O_WRONLY | O_SYNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH);
...
int jsFileWriter::writeSingleProperty(std::string datasetPath, std::string fileName, std::string propertyName, std::string propertyValue) {
  time_t rawtime;
  time(&rawtime);
  std::string comments = "# www.javaseis.org - JavaSeis File Properties " + JS_VERSION + "\n# " + ctime(&rawtime);

  DIR *dr;
  if((dr = opendir(datasetPath.c_str())) == NULL) mkdir(datasetPath.c_str(), 0777);
  else closedir(dr); //close all directory
...
int jsFileWriter::writeHeaderBuffer(long offset, char *buf, long buflen) {
  ...
  for(int extInd = lowInd; extInd <= upInd; extInd++) {
    ...
    // int curr_trhfd = ::open(fname.c_str(), O_WRONLY | O_SYNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | O_DIRECT);
    int curr_trhfd = ::open(fname.c_str(), O_WRONLY | O_SYNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH);
    if(curr_trhfd < 0) {
      return JS_WARNING;
    }
    ...
    long bytesWritten = wrapIOFull(pwrite, curr_trhfd, &buf[buflen - rest_buflen], bytes2write, loc_offset_trHeader);
...
int jsFileWriter::writeTraceBuffer(long offset, char *buf, long buflen) {
  ...
  for(int extInd = lowInd; extInd <= upInd; extInd++) {
    ...
    //int curr_trffd = ::open(fname.c_str(), O_CREAT | O_WRONLY | O_SYNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | O_DIRECT);
    int curr_trffd = ::open(fname.c_str(), O_CREAT | O_WRONLY | O_SYNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH);
    if(curr_trffd < 0) {
      ERROR_PRINTF(jsFileWriterLog, "Can't create %s for writing!", fname.c_str());
      return JS_WARNING;
    }
    ...
    ::close(curr_trffd);
    if(bytesWritten != bytes2write) {
      ERROR_PRINTF(jsFileWriterLog, "%s: bytes written(%ld) do not match bytes to write(%ld)!", fname.c_str(), bytesWritten, bytes2write);
      return JS_WARNING;
    }
...
int jsFileWriter::writeFrames(int *posLogical, float *frames, int nFrames) {
  //     position[m_fileProps->numDimensions-2]=0;
  long frameIndex = getFrameIndex(posLogical);
...
int jsFileWriter::writeFrames(long frameIndex, float *frames, int nFrames) {
  ...
  if(frameIndex < 0 || frameIndex + nFrames - 1 >= m_TotalNumOfFrames) {
    ERROR_PRINTF(jsFileWriterLog, "Invalid frame index. [%ld,%ld] must be in [0,%ld)", frameIndex, frameIndex + nFrames - 1,
                 m_TotalNumOfFrames);
    return JS_USERERROR;
  }
...
long jsFileWriter::getFrameIndex(int *posLogical) { // *input position must be in logical corrdinates
  int numAxis = m_fileProps->numDimensions;
...
int jsFileWriter::writeSubFrames(std::string hdrSubFrame, int *posLogical, float *frame, char *headbuf, int nr, bool verbose) {
  auto hdr = getHdrEntry(hdrSubFrame);

  vector<int> vals;
  vector<int> counts;
  vector<int> valbuf(nr);
  for(int ir = 0; ir < nr; ir++) {
    int val = hdr.getIntVal(&headbuf[(size_t)ir * m_headerLengthBytes]);
    valbuf[ir] = val;
    auto pos = std::find(vals.begin(), vals.end(), val);
    int j = pos - vals.begin();
    if(pos == vals.end()) vals.push_back(val), counts.push_back(1);
    else counts[j] += 1;
  }

  int ngroup = counts.size();
  vector<float> data((size_t)nr * m_numSamples);
  vector<char> hdrbuf((size_t)nr * m_headerLengthBytes);
  for(int j = 0; j < ngroup; j++) {
    for(int ir = 0, irSub = 0; ir < nr; ir++) {
      if(valbuf[ir] == vals[j]) {
        memcpy(&data[(size_t)irSub * m_numSamples], frame + (size_t)ir * m_numSamples, sizeof(float) * m_numSamples);
        memcpy(&hdrbuf[(size_t)irSub * m_headerLengthBytes], headbuf + (size_t)ir * m_headerLengthBytes,
               sizeof(char) * m_headerLengthBytes);
        irSub++;
      }
    }
    posLogical[2] = vals[j];
    if(verbose) printf("Writing subframe (logic value=%d)\n", vals[j]), fflush(stdout);
    writeFrame(posLogical, &data[0], &hdrbuf[0], counts[j]);
  }

  return nr;
}

int jsFileWriter::writeFrame(int *posLogical, float *frame, char *headbuf, int numLiveTraces) {
  //     position[m_fileProps->numDimensions-2]=0;
  long frameIndex = getFrameIndex(posLogical);
...
int jsFileWriter::writeFrame(long frameIndex, float *frame, char *headbuf, int numLiveTraces) {
  ...
  if(numLiveTraces > 0) {  // we do  need write anything.
    ...
    if(m_bisFloat) {     //if float, there is no need to uncompress, we can write directly  (should be faster)
      ...
      for(size_t i = 0; i < m_trBufferArrayLen; i++)
        traceBufferArray[i] = 0;
      ...
      if(m_bSeisPEG_data) {
        ...
        if(headbuf != NULL) {
          ...
          bytesInFrame = seispegCompressor->compress((float*)frame, numLiveTraces, seispegHeaderBuffer, m_headerLengthWords,
                                                     traceBufferArray);
  ...
  if(m_trMap && bWriteTraceMap == true) {
...
int jsFileWriter::writeFrameHeader(int *posLogical, char *headbuf) {
  //     position[m_fileProps->numDimensions-2]=0;
  long frameIndex = getFrameIndex(posLogical);
...
int jsFileWriter::leftJustify(float *frame, char *headbuf, long numTraces) {
...
float* jsFileWriter::allocFrameBuf(int nframes) {
  if(!m_bInit) {
    ERROR_PRINTF(jsFileWriterLog, "Properties must be initialized first");
    return NULL;
  }
  return new float[(size_t)nframes * m_numSamples * m_numTraces]();
}

char* jsFileWriter::allocHdrBuf(bool initVals, int nframes) {
  if(!m_bInit) {
    ERROR_PRINTF(jsFileWriterLog, "Properties must be initialized first");
    return NULL;
  }
  char *hdrBuf = new char[(size_t)nframes * m_headerLengthBytes * m_numTraces]();

  if(initVals) { // init with SeisSpace standard values
    float TFULL_E = m_fileProps->physicalOrigins[0] + (m_fileProps->axisLengths[0] - 1) * m_fileProps->physicalDeltas[0];
...
catalogedHdrEntry jsFileWriter::getHdrEntry(std::string _name, bool must_exist) const {
  return m_traceProps->getHdrEntry(_name, must_exist);
}
...
catalogedHdrEntry jsFileWriter::getAxisHdrEntry(int _axisInd) const {
  if(_axisInd >= 0 && _axisInd < m_numDim) {
    return getHdrEntry((m_fileProps->axisLabels[_axisInd]).getName(), true);
  } else {
    assertion(false, "Invalid axis index %d. Must be between 0 and %d", _axisInd, m_numDim);

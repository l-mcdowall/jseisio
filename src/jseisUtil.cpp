...
oJseis3D::oJseis3D(const char *fname0, int n1, int n2, int n3, float d1, float d2, float d3, int io1, float o2, float o3, int io2, int inc2,
                   int io3, int inc3, DataFormat dataFormat, bool is_depth, float t0)
    : _n1(n1),
      _n2(n2),
      _n3(n3),
      _i3(0),
  ...
  jsWrt.initGridDim(3);
  if (_is_depth)
    jsWrt.initGridAxis(0, "DEPTH", "METERS", "DEPTH", _n1, _io1, 1, _o1, _d1);
  else
...
oJseis3D::~oJseis3D() { delete[] hdbuf; }

oJseisND::~oJseisND() { Close(); }
...
float *oJseisND::allocFrameBuf() { return jsWrt.allocFrameBuf(); }

char *oJseisND::allocHdrBuf(bool initVals) { return jsWrt.allocHdrBuf(initVals); }
...
oJseisND::oJseisND(string fname0, vector<int> lengths, vector<int> logicalOrigins, vector<int> logicalDeltas,
                   vector<double> physicalOrigins, vector<double> physicalDeltas, std::vector<string> axisHdrs, DataFormat dataFormat,
                   bool is_depth, std::vector<string> *extendHdrNames, std::vector<string> *extendHdrTypes)
    : oJseisND(fname0, lengths.size(), &lengths[0], &logicalOrigins[0], &logicalDeltas[0], &physicalOrigins[0], &physicalDeltas[0],
               axisHdrs, dataFormat, is_depth, extendHdrNames, extendHdrTypes) {}
oJseisND::oJseisND(string fname0, vector<int> lengths, vector<int> logicalOrigins, vector<int> logicalDeltas,
                   vector<double> physicalOrigins, vector<double> physicalDeltas, std::vector<string> axisHdrs,
                   std::vector<string> extendHdrNames, std::vector<string> extendHdrTypes, DataFormat dataFormat, bool is_depth)
    : oJseisND(fname0, lengths.size(), lengths.data(), logicalOrigins.data(), logicalDeltas.data(), physicalOrigins.data(),
               physicalDeltas.data(), axisHdrs, dataFormat, is_depth, &extendHdrNames, &extendHdrTypes) {}
oJseisND::oJseisND(string fname0, int ndim, int *lengths, int *logicalOrigins, int *logicalDeltas, double *physicalOrigins,
                   double *physicalDeltas, std::vector<string> axisHdrs, DataFormat dataFormat, bool is_depth,
  ...
  if (!(extendHdrNames == NULL ||
        (extendHdrNames != NULL && extendHdrTypes != NULL && (*extendHdrNames).size() == (*extendHdrTypes).size()))) {
  ...
  if (extendHdrNames != NULL && (*extendHdrNames).size() > 0) {
    ...
    for (int i = 0; i < nhs; i++) {
      ...
      if ((*extendHdrTypes)[i] == "FLOAT") fExtHdrs.push_back(jsWrt.getHdrEntry((*extendHdrNames)[i]));
    }
  }

  if (is_depth)
    jsWrt.initGridAxis(0, axisHdr1, "METERS", "DEPTH", _n1, _io1, _inc1, _o1, _d1);
  else
    jsWrt.initGridAxis(0, axisHdr1, "MILLISECONDS", "TIME", _n1, _io1, _inc1, _o1, _d1);
...
int oJseisND::write_frame(float *data, int iframe, int ivolume, int ihypercube, const vector<float> extHdrs) {  // index start from 0
  ...
  size_t nExts = fExtHdrs.size();
  if (!extHdrs.empty())
    assertion(extHdrs.size() == nExts * _n2, "JS write_frame: extHdrs.size()=%ld != fExtHdrs.size()*n2(%ld*%d)", extHdrs.size(), nExts,
              _n2);
  for (int i2 = 0; i2 < _n2; i2++) {
    itrcTypeHdr.setIntVal(&hdbuf[i2 * traceheaderSize], 1);  // default 1 is OK
    ...
    if (!extHdrs.empty())
      for (size_t iExt = 0; iExt < nExts; iExt++) fExtHdrs[iExt].setFloatVal(&hdbuf[i2 * traceheaderSize], extHdrs[i2 * nExts + iExt]);
  }
...
string jseisUtil::fullname(const char *fname0, string &descname) {
  ...
  if (fname.rfind('/') != fname.length() - 1 && fname.rfind(".js") != fname.length() - 3 && fname.rfind(".VID") != fname.length() - 4 &&
      fname.rfind(".vid") != fname.length() - 4)
    fname = fname + ".js";  // if ends with / then do not append .js
  ...
  // std::transform(fname.begin() + pos0, fname.end(), fname.begin() + pos0, ::tolower); // convert file name corresponding to descname to
  // lower case
...
int jseisUtil::save_zxy(const char *fname, vector<vector<float> > data, float dz, float dx, float dy, int iz0, float x0, float y0, int io2,
  ...
  for (auto &trc : data) nz = std::max(nz, trc.size());
  vector<float> buf(nz * nx, 0);
  for (size_t ix = 0; ix < nx; ix++) memcpy(&buf[(ix * nz)], &data[ix][0], sizeof(float) * data[ix].size());
...
int jseisUtil::check_vol(jsFileReader &js, const char *fname0, vector<int> &axisLens, vector<int> &logicOrigs, vector<int> &logicDeltas,
  ...
  for (int i = 0; i < ndim; i++) axisLens[i] = js.getAxisLen(i);
...
oJseisShots::oJseisShots(const char *fname0, bool recreate, int n1, int n2, int n3, float dt_seconds, float d2, float d3, int io1, float o2,
                         float o3, int io2, int inc2, int io3, int inc3, jsIO::DataFormat dataFormat)
    : _n1(n1),
      _n2(n2),
      _n3(n3),
      _i3(0),
  ...
  if (recreate ||
      !jseisUtil::file_exists(fname0)) {  // TODO: if data exists and overwrite==false, check the data contexts and headers are the same!!
    ires = jsWrt.writeMetaData();
    if (ires != JS_OK) {
      fprintf(stderr, "Error writing meta data!\n");
      return;
...
int oJseisShots::write_frame(int iframe, float *data, char *hdr, int ntrc) { return jsWrt.writeFrame(iframe, data, hdr, ntrc); }

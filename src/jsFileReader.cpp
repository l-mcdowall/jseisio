...
int jsFileReader::Init(const std::string _jsfilename, const int _NThreads, int wait) {
  ...
  /*
   ...
   tmp1name = tmp1name.substr(0, pos0);
   DIR *dirp = opendir(tmp1name.c_str());
   struct dirent *dp;
  ...
  m_frameSize = (long)m_numTraces * (long)m_compess_traceSize;
  ...
  TRACE_PRINTF(jsFileReaderLog, "traceLen=%d, TotalNumOfFrames=%ld, TotalNumOfTraces=%ld", m_compess_traceSize, m_TotalNumOfFrames,
               m_TotalNumOfLiveTraces);
...
int jsFileReader::readTraceHeader(vector<int> posPartial, char *headbuf) {
  vector<int> position = fillPositionPartial(posPartial);
  return readTraceHeader(&position[0], headbuf);
}

int jsFileReader::readFrameHeader(vector<int> posPartial, char *headbuf) {
  vector<int> position = fillPositionPartial(posPartial);
  return readFrameHeader(&position[0], headbuf);
}

int jsFileReader::readTrace(vector<int> posPartial, float *trace) {
  vector<int> position = fillPositionPartial(posPartial);
  return readTrace(&position[0], trace);
}

int jsFileReader::readFrame(vector<int> posPartial, float *frame, char *headbuf) {
  vector<int> position = fillPositionPartial(posPartial);
  return readFrame(&position[0], frame, headbuf);
}

int jsFileReader::readRawFrames(vector<int> posPartial, int NFrames, char *rawframe, int *numLiveTraces) {
  vector<int> position = fillPositionPartial(posPartial);
  return readRawFrames(&position[0], NFrames, rawframe, numLiveTraces);
}

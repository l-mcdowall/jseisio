...
public:
  oJseis3D(const char *fname, int n1, int n2, int n3, float d1 = 1.0f, float d2 = 1.0f, float d3 = 1.0f, int io1 = 0, float o2 = 0,
           float o3 = 0, int io2 = 1, int inc2 = 1, int io3 = 1, int inc3 = 1,
           jsIO::DataFormat dataFormat = jsIO::DataFormat::COMPRESSED_INT16, bool is_depth = true, float t0 = 0);
  virtual ~oJseis3D();
...
public:
  ...
  vector<jsIO::catalogedHdrEntry> fExtHdrs;
  int _ndim = 3;
...
public:
  ...
  oJseisND(string fname0, vector<int> lengths, vector<int> logicalOrigins, vector<int> logicalDeltas, vector<double> physicalOrigins,
           vector<double> physicalDeltas, std::vector<string> axisHdrs, std::vector<string> extendHdrNames,
           std::vector<string> extendHdrTypes, DataFormat dataFormat = jsIO::DataFormat::COMPRESSED_INT16, bool is_depth = true);
  virtual ~oJseisND();
  void Close();
  int write_frameHeader(char *hdr, int iframe, int ivolume = 0, int ihypercube = 0);  // index start from 0
  int write_frame(float *data, int iframe, int ivolume = 0, int ihypercube = 0,
                  const vector<float> extHdrs = vector<float>{});                            // index start from 0
...
public:
  oJseisShots(const char *fname, bool recreate, int n1, int n2, int n3, float dt_seconds = 0.004f, float d2 = 1.0f, float d3 = 1.0f,
              int io1 = 0, float o2 = 1, float o3 = 1, int io2 = 1, int inc2 = 1, int io3 = 1, int inc3 = 1,
...
public:
  ...
  static int save_zxy(const char *fname, float *data, int nz, int nx, int ny, float dz = 10.0f, float dx = 50.0f, float dy = 50.0f,
                      int iz0 = 0, float x0 = 0.0f, float y0 = 0.0f, int io2 = 1, int inc2 = 1, int io3 = 1, int inc3 = 1,
                      jsIO::DataFormat dataFormat = jsIO::DataFormat::COMPRESSED_INT16, bool is_depth = true, float t0 = 0);
  static int save_zxy(const char *fname, vector<vector<float>> data, float dz = 10.0f, float dx = 50.0f, float dy = 50.0f, int iz0 = 0,
                      float x0 = 0.0f, float y0 = 0.0f, int io2 = 1, int inc2 = 1, int io3 = 1, int inc3 = 1,
                      jsIO::DataFormat dataFormat = jsIO::DataFormat::COMPRESSED_INT16, bool is_depth = true, float t0 = 0);

...
public:
  ...
  void add_secondary_path(std::string _path) {
    m_virtualFolders.push_back(_path);
  }

  void set_secondary_path(std::string _path) {
    m_virtualFolders.clear();
    if(_path.length() > 0) m_virtualFolders.push_back(_path);
  }
  ...
  int Initialize(const std::string _filename); // one-step initialize with existing jseis data and continue to write to the same data

  // change the grid of axis after Init( jsFileReader* jsReader) only, before Initialize and writeMetaData
  int updateGridAxis(int axisInd, long length, long logicalOrigin, long logicalDelta, double physicalOrigin, double physicalDelta);
   ...
   * It is up to the user to free the buffer afterwards
   */
  char* allocHdrBuf(bool initVals = true, int nframes = 1);

  ///Returns the size of an array (in bytes) that will be allocated by allocHdrBuf() function
  long getHdrBufSize() {
    return m_headerLengthBytes * m_numTraces;
  }

  ///Get access to header-word / property with a name _name
  catalogedHdrEntry getHdrEntry(std::string _name, bool must_exist = false) const;
  ...
  unsigned long getIOBufferSize() const {
  ...
  JS_BYTEORDER getByteOrder() const {
    return m_byteOrder;
  }
  ...
  int indexToLogical(int *posIndex) const; // *input position must be in index, and will convert to logical corrdinates

  int logicalToIndex(int *posLogical) const; // *input position must be in logical corrdinates and will convert to index
   ...
   */
  int leftJustify(float *frame, char *headbuf, long numTraces);
   ...
   *
   */
  int writeFrame(int *posLogical, float *frame, char *headbuf = NULL, int numLiveTraces = -1);

  /**
   * @brief Writes frame to the dataset
   * @param hdrSubFrame todo
   * @param position logical position, WILL BE CHANGED!
   * @param frame the frame to be written
   * @param headbuf the corresponding frame header. If not NULL then it will be written in
   *                the corresponding position in TraceHeader(s)
   * @param numLiveTraces the number of live traces in the frame. If positive, then it will be written in
   ...
   *
   */
  int writeSubFrames(std::string hdrSubFrame, int *posLogical, float *frame, char *headbuf, int numLiveTraces, bool verbose = false);
   ...
   */
  int writeFrameHeader(int *posLogical, char *headbuf);
   ...
   */
  int writeFrames(int *posLogical, float *frames, int nFrames);
  ...
  TraceProperties* getTraceProps() const {
    return m_traceProps;
  }

  long getFrameIndex(int *posLogical);    // position must be in logical coordinate
...
private:
  ...
  size_t m_traceFileSize { };
  size_t m_headerFileSize { };
  ...
  size_t m_frameHeaderSize { };
  int m_headerLengthWords { };
  int m_seispegPolicy { };

  size_t m_trBufferArrayLen { };
...
private:
  int writeSingleProperty(std::string datasetPath, std::string fileName, std::string propertyName, std::string propertyValue);
